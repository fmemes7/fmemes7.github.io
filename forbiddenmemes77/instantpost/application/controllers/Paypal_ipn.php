...
class Paypal_ipn extends CI_Controller
{
    ...
    public function ipn_notify()
    {
        ...
        $prev_payment_info = $this->common->readData('transaction_history',$simple_where,$select,$join='',$limit='1',$start=0,$order_by='ID DESC',$group_by='');
       ...
       $validity=$package_data[0]["validity"];
        ...
        $this->common->createData('transaction_history', $insert_data);
        ...
        $product_short_name = $this->config->item('itemShortName');
        $from = $this->config->item('companyEmail');
        $mask = $this->config->item('itemName');
        $subject = "Payment Confirmation";
        $where = array();
        $where['where'] = array('id'=>$user_id);
        $user_email = $this->common->readData('users',$where,$select='');
        ...
        //send mail to user
        $this->_mailSender($from, $to, $subject, $message, $mask, $html=0);
        $to = $from;
    ...
    function _mailSender($from = '', $to = '', $subject = '', $message = '', $mask = "", $html = 0, $smtp = 1)
    {
        if ($from!= '' && $to!= '' && $subject!='' && $message!= '') {
            ...
            if ($smtp == '1') {
                ...
                $email_config_details = $this->common->readData("smtp_configuration", $where2, $select = '', $join = '', $limit = '', $start = '',
                                                        $group_by = '', $num_rows = 0);
