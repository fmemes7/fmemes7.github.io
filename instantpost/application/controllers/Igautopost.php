...
class Igautopost extends Main
{
	...
	public function __construct()
    {
        ...
        if($this->session->userdata('userType') != 'Admin' && !in_array(501,$this->moduleAccess)) {
        	redirect('main/loginPage', 'location'); 
        }
    ...
    public function addAutoPost()
    {
    	$data['body'] = 'shadowpostig/auto_post/add_auto_post';
    	$data['pageTitle'] = 'Auto Post';
    ...
    public function addAutoPostAction()
    {
        if(!$_POST) {
            exit();
        }
        ...
        if(count($post_to_account)>0) {
            ...
            foreach ($account_info as $key => $value) {
                ...
                if($schedule_type=="now") {
                    ...
                    $this->common->createData("instagram_auto_post",$insert_data_batch[$count]);
                }
                $count++;
            }
        }
        if($schedule_type=="now") {
            ...
            if($this->db->insert_batch("instagram_auto_post",$insert_data_batch)) {
    ...
    public function uploadImageOnly()
    {
        ...
        $ret = [];
        $output_dir = FCPATH.'upload/autopost';
