...
class Payment extends Main
{
    ...
    */
    ...
    public function payment_dashboard_admin()
    {
        ...
        $total_user = $this->common->readData('users',$total_where_users='',$select=array('count(id) as total_user'));
        ...
        $this_month_user = $this->common->readData('users',$this_month_where,$select=array('count(id) as total_user'));
        if(!empty($this_month_user))
            $data['this_month_total_user'] = $this_month_user[0]['total_user'];
        else
            $data['this_month_total_user'] = 0;
        $total_paid_amount = $this->common->readData('transaction_history',$where='',$select=array('sum(paid_amount) as total_paid_amount'));
        ...
        $this_month_paid_amount = $this->common->readData('transaction_history',$this_month_paid_where,$select=array('sum(paid_amount) as total_paid_amount'));
        ...
        $today_user = $this->common->readData('users',$where_today_user,$select=array('count(id) as total_user'));
        if(!empty($today_user))
        ...
        $today_paid_amount = $this->common->readData('transaction_history',$today_paid_where,$today_select=array('sum(paid_amount) as total_paid_amount'));
    ...
    public function admin_payment_history()
    {
        ...
        $info = $this->common->readData($table, $where='', $select = '');        
        $total_paid_amount = 0;
    ...
    public function admin_payment_history_data()
    {
        ...
        $total_rows_array = $this->common->countRow($table, $where, $count = "id");
    ...
    public function member_payment_history()
    {
        ...
        $data['page_title'] = $this->lang->line("payment history");
        $data["packages"]=$this->_paymentPackage();        
        $config_data=$this->common->readData("payment_configuration");
        $data["currency"]=$config_data[0]["currency"];
        $this->_mainView($data);
    }
    ...
    public function payment_button()
    {
        ...
        if($_POST)
        {
            ...
            $package_data=$this->common->readData("package",$where=array("where"=>array("package.id"=>$package_id)));
            ...
            $payment_config = $this->common->readData('payment_configuration',$where,$select='');
            ...
            $this->paypal_class->mode="sandbox";
            ...
            $this->paypal_class->product_name=$this->config->item("itemName")." : ".$package_name." (".$package_validity." days)";
            ...
            /*****  Stripe Button ******/
            if($stripe_secret!=""){
            $this->stripe_class->description=$this->config->item("itemName")." : ".$package_name." (".$package_validity." days)";
    ...
    public function package_settings()
    {
        ...
        $data['payment_config']=$this->common->readData('payment_configuration');
        $this->_mainView($data);  
    }
    ...
    {           
        ...
        $total_rows_array=$this->common->countRow($table="package",$where='',$count="package.id");
        $total_result=$total_rows_array[0]['total_rows'];            
        echo convertToGridData($info,$total_result);
    }
    ...
    {        
        if($this->session->userdata('loggedIn') == 1 && $this->session->userdata('userType') != 'Member') 
        redirect('main/loginPage', 'location');
        $info = $this->common->readData($table="modules", $where="", $select = "view_usage_log.*,modules.module_name,modules.id as module_id,limit_enabled,extra_text",$join=array('view_usage_log'=>"view_usage_log.module_id=modules.id AND user_id =".$this->session->userdata("userId").",left"),$limit='',$start=NULL,$order_by='module_name asc');        
        $package_info=$this->session->userdata("packageInfo");
