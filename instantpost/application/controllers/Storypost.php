...
class Storypost extends Main
{
	...
	public function __construct()
    {
        ...
        if($this->session->userdata('userType') != 'Admin' && !in_array(502,$this->moduleAccess)) {
        	redirect('main/loginPage', 'location'); 
        }
    ...
    {
    	$data['body'] = 'shadowpostig/story_post/add_auto_post';
    	$data['pageTitle'] = 'Story Post';
    ...
    public function addStoryPostAction()
    {
        if(!$_POST) {
            exit();
        }
        ...
        if($video_url!="") {
            if(strpos($video_url, 'youtube.com') !== false) {
                ...
                if($videoId!="") {
                    $video_url = $this->shadowpost_uo_library->getYoutubeVideoUrl($videoId);
        ...
        if(count($post_to_account)>0) {
            ...
            foreach ($account_info as $key => $value) {
                ...
                if($schedule_type=="now") {
                    ...
                    $this->common->createData("instagram_story_post",$insert_data_batch[$count]);
                }
                $count++;
            }
        }
        if($schedule_type=="now") {
            ...
            if($this->db->insert_batch("instagram_story_post",$insert_data_batch)) {
    ...
    public function uploadImageOnly()
    {
        ...
        $ret = [];
        $output_dir = FCPATH.'upload/storypost';
    ...
    public function uploadVideoThumb()
    {
        ...
        if (isset($_FILES['myfile'])) {
            ...
            $filename = 'videothumb_'.$this->userId.'_'.time().substr(uniqid(mt_rand(), true), 0, 6).'.'.$ext;
            move_uploaded_file($_FILES['myfile']['tmp_name'], $output_dir.'/'.$filename);
            $ret[] = $filename;
            echo json_encode($filename);
        }
